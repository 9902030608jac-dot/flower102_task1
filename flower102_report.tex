...
本报告中的主要评价指标为 Top-1 Accuracy，同时补充 macro mAP 作为类别级 one-vs-rest 排序质量指标。由于本任务是单标签 102 类分类，Accuracy 仍作为模型选择的主要指标；mAP 用于辅助分析模型对各类别置信分数的排序能力，并满足训练过程可视化要求。
...
训练阶段采用随机增强策略，以提高模型对尺度、位置和颜色变化的鲁棒性。具体预处理包括 \texttt{RandomResizedCrop(224, scale=(0.65,1.0), ratio=(0.75,1.333))}、\texttt{RandomHorizontalFlip(p=0.5)}、轻量 \texttt{ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.05)}、\texttt{ToTensor} 以及基于 ImageNet 均值和标准差的归一化。Flowers102 训练集规模较小，随机增强可以人为制造合理扰动，减少模型对固定背景、拍摄角度、主体尺度和光照条件的记忆；但花卉分类又高度依赖颜色、花瓣形态和纹理，因此本实验没有使用强旋转、过强颜色扰动、CutMix、MixUp、AutoAugment 或 RandAugment，避免增强破坏类别判别特征。验证和测试阶段采用确定性的 Resize、CenterCrop、ToTensor 和 Normalize，保证评估结果不受随机增强影响。ImageNet 归一化参数为：
\begin{equation}
...
本实验在 ResNet-18 Baseline 基础上手动加入两类注意力模块：SE-block 和 CBAM。最终代码将注意力模块插入每个 ResNet BasicBlock 的 residual branch 末端，即 \verb|conv1 -> bn1 -> relu -> conv2 -> bn2 -> SE/CBAM -> add shortcut -> relu|。这种插入方式让注意力作用在卷积特征图上，并在 shortcut 相加之前重标定残差分支输出，避免将注意力模块错误放在整个网络末端或最终 logits 之后。
...
\section{实验设置}
实验统一使用输入尺寸 \(224\times224\)。ResNet-18 系列 batch size 为 64，ResNet-34 系列 batch size 为 32。ResNet-34 的参数量和显存占用高于 ResNet-18，因此采用较小 batch size。优化器为 AdamW，损失函数为 Cross Entropy Loss，学习率调度器为 CosineAnnealingLR，权重衰减为 \(1\times10^{-4}\)，随机种子为 42。评价指标为 Top-1 Accuracy 与 macro mAP。
...
注意力机制实验需要基于 BasicBlock 内部插入版本重新训练并重新汇报。重训时，WandB 将逐 epoch 记录训练/验证 loss、验证集 Accuracy 与 macro mAP；同时，优化器会将预训练 backbone、新增 attention module 和分类头拆成不同学习率组，以减少随机初始化 attention 参数学习不足或破坏预训练 backbone 的风险。

\subsection{训练曲线与收敛性分析}
训练过程中记录每个 epoch 的 \verb|train_loss|、\verb|val_loss|、\verb|train_acc|、\verb|val_acc| 和 \verb|val_map|，并同步写入 WandB。代码同时基于 \verb|metrics.csv| 自动绘制本地曲线 \verb|training_curves.png| 和 \verb|wandb_training_curves.png|。这些曲线展示训练集与验证集 loss，以及验证集 Accuracy / mAP 的变化；WandB run 本地缓存保存在 \verb|outputs/logs/{experiment_name}/|，本地图片保存在 \verb|outputs/figures/{experiment_name}/wandb_training_curves.png|，可作为报告中的可视化截图来源。
...
注意力机制实验表明，SE 和 CBAM 需要基于 BasicBlock 内部插入方式重新训练评估。当前代码将注意力模块插入每个 BasicBlock 的 `conv2 -> bn2` 之后、shortcut 相加之前，避免将 attention 错误作用在整段 stage 输出或最终 logits 上。新增模块随机初始化，小数据集上额外参数仍可能增加优化难度，因此需要结合 WandB 曲线、验证集 Accuracy/mAP 和测试集结果综合分析。混淆矩阵后处理进一步给出了 normalized confusion matrix、top confused pairs 和 per-class accuracy，使错误分析不只停留在热力图展示层面。后续工作可从更系统的数据增强、更细粒度学习率搜索、早停机制、更强 backbone、identity-like attention initialization 与更稳定的 attention 插入方式等方向继续改进。

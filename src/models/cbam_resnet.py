...
def _attach_cbam_to_basic_blocks(model: nn.Module, reduction: int) -> None:
    for module in model.modules():
        if isinstance(module, BasicBlock):
            channels = module.bn2.num_features
            module.attention = CBAMBlock(channels, reduction)
...
def build_cbam_resnet(arch: str, num_classes: int = 102, pretrained: bool = True, reduction: int = 16) -> nn.Module:
    """Build ResNet with CBAM inserted inside every BasicBlock before residual add."""
    model = build_resnet_baseline(arch, num_classes=num_classes, pretrained=pretrained)
    _attach_cbam_to_basic_blocks(model, reduction)
    return model
